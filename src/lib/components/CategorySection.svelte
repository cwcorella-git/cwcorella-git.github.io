<script lang="ts">
	...
	$derived subcategories = links.reduce<Subcategory[]>((acc, link) => {
	...
	$derived hasSubcategories = subcategories.length > 1 || (subcategories.length === 1 && subcategories[0].name !== 'Other');
</script>
